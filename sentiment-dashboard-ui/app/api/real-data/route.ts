...
async function fetchRedditData(query: string) {
  try {
    const response = await fetch(`https://www.reddit.com/search.json?q=${encodeURIComponent(query)}&limit=10&sort=hot`, {
      headers: {
...
async function fetchNewsData(query: string) {
  try {
    const response = await fetch(`https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&sortBy=publishedAt&pageSize=5&apiKey=demo`, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; SentimentalAI/1.0)'
      }
    })
